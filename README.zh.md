...
移动后重新检查内容与稳定文件身份，确认归属后才删除。错误替代文件会尽量原子恢复到
原路径；恢复受阻时保留在 quarantine 并 fail closed。自动回滚要求 Windows
no-replace rename 或 Linux `renameat2(RENAME_NOREPLACE)`；其他平台不降级执行。
